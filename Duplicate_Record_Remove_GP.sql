...
WHILE 1=1 BEGIN
	...
	UNION SELECT MAX(idfAPVendorKey), idfVendorID AS ID,idfPTICompanyKey,'APVendor' FROM dbo.APVendor WITH (NOLOCK) GROUP BY idfVendorID,idfPTICompanyKey HAVING COUNT(*) > 1
	...
	UNION SELECT MAX(idfIVItemKey), idfItemID AS ID,idfPTICompanyKey,'IVItem' FROM dbo.IVItem WITH (NOLOCK) GROUP BY idfItemID,idfPTICompanyKey HAVING COUNT(*) > 1
	UNION SELECT MAX(idfIVSiteKey), idfSiteID AS ID,idfPTICompanyKey,'IVSite' FROM dbo.IVSite WITH (NOLOCK) GROUP BY idfSiteID,idfPTICompanyKey HAVING COUNT(*) > 1
	...
	UNION SELECT MAX(idfWCAddressKey), idfAddressID AS ID,idfPTICompanyKey,'WCAddress' FROM dbo.WCAddress WITH (NOLOCK) GROUP BY idfAddressID,idfPTICompanyKey HAVING COUNT(*) > 1
	UNION SELECT MAX(idfWCPaymentTermKey), idfPaymentTermID AS ID,idfPTICompanyKey,'WCPaymentTerm' FROM dbo.WCPaymentTerm WITH (NOLOCK) GROUP BY idfPaymentTermID,idfPTICompanyKey HAVING COUNT(*) > 1
	UNION SELECT MAX(idfWCShippingMethodKey), idfShippingMethodID AS ID,idfPTICompanyKey,'WCShippingMethod' FROM dbo.WCShippingMethod WITH (NOLOCK) GROUP BY idfShippingMethodID,idfPTICompanyKey HAVING COUNT(*) > 1
