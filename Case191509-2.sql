...
--6/29/2018
select idfWCSecurityKey,* from PAProject where idfProjectID='7080035001'
select idfWCSecurityKey,* from PAProjectPhase where idfPAProjectKey=75296069 --257
select * from WCSecurity where idfWCSecurityKey=967
select * from WCLineUp where idfLineUpID like '%*APR_967%' 
select * from WCRRGroup where idfDescription like '%_967%'
select * from WCListHdr where idfListID like '%_967%'
select * from WCListDtl where idfWCListHdrKey=58

select * from WCLineUpSec where idfWCSecurityKey =967
select * from WCRRGroupLineUp where idfWCLineUpKey =115

--Not Matching Detail of the NFL
select * from PAProjectPhase detail
JOIN WCListDtl PhaseList (NOLOCK) ON PhaseList.idfWCListHdrKey = 58 AND Detail.idfPAProjectPhaseKey = PhaseList.idfCodeKey  WHERE Detail.idfWCLineUpKey IS NULL 

select idfPAProjectPhaseKey,* from PAProjectPhase
where idfPAProjectPhaseKey not in (
select idfCodeKey from WCListDtl where idfWCListHdrKey=58)
and idfPAProjectKey in (select idfPAProjectKey from PAProject where idfProjectID='7080035001')
