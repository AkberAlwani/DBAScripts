---spEXPExpenseSheetHdrCreateGLJournal
BEGIN TRAN
...
,@xnidfEXPExpenseSheetHdrKey INT
,@xochErrSP					CHAR(32)      	= ''	
,@xonErrNum					INT          	= 0		
,@xostrErrInfo				VARCHAR(255) 	= ''	
,@xnPostGL                      INT = 1		
--- HERE WE CAN START  ONE AT THE TIME TO VERIFY RECORD IS CREATED
SELECT @xnidfEXPExpenseSheetHdrKey=13138
--- 
...
(
	...
	,idfGLSegmentDtlKey01 INT NULL
	,idfGLSegmentDtlKey02 INT NULL
	,idfGLSegmentDtlKey03 INT NULL
	,idfGLSegmentDtlKey04 INT NULL
	,idfGLSegmentDtlKey05 INT NULL
	,idfGLSegmentDtlKey06 INT NULL
	,idfGLSegmentDtlKey07 INT NULL
	,idfGLSegmentDtlKey08 INT NULL
	,idfGLSegmentDtlKey09 INT NULL
	,idfGLSegmentDtlKey10 INT NULL
	,idfGLSegmentDtlKey11 INT NULL
	,idfGLSegmentDtlKey12 INT NULL
	,idfGLSegmentDtlKey13 INT NULL
	,idfGLSegmentDtlKey14 INT NULL
	,idfGLSegmentDtlKey15 INT NULL
)

SELECT @strEXPNumber			= idfEXPExpenseSheetHdrHistKey  
FROM dbo.EXPExpenseSheetHdrHist WITH (NOLOCK) 	
WHERE idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey
...
DECLARE @tblGLJournalDtl TABLE (idfTableLinkKey INT,idfTableLinkName VARCHAR(80), idfAmount NUMERIC(19,5), idfAmountHome NUMERIC(19,5), idfType VARCHAR(10), idfGLAccountKey INT, idfGLReferenceKey INT NULL, idfGLAccountKey_PAY INT NULL, idfFlagEncumbrance INT NULL, idfFlagOverlayGL INT, idfFlagOverlay_PAY INT, idfWCOrganizationKey INT
,idfGLSegmentDtlKey01 INT NULL
,idfGLSegmentDtlKey02 INT NULL
,idfGLSegmentDtlKey03 INT NULL
,idfGLSegmentDtlKey04 INT NULL
...
SELECT 
	...
	,RD.idfPercent
	,RD.idfGLAccountKey
	,RD.idfGLSegmentDtlKey01
	,RD.idfGLSegmentDtlKey02
	,RD.idfGLSegmentDtlKey03
	,RD.idfGLSegmentDtlKey04
	,RD.idfGLSegmentDtlKey05
	,RD.idfGLSegmentDtlKey06
	,RD.idfGLSegmentDtlKey07
	,RD.idfGLSegmentDtlKey08
	,RD.idfGLSegmentDtlKey09
	,RD.idfGLSegmentDtlKey10
	,RD.idfGLSegmentDtlKey11
	,RD.idfGLSegmentDtlKey12
	,RD.idfGLSegmentDtlKey13
	,RD.idfGLSegmentDtlKey14
	,RD.idfGLSegmentDtlKey15
...
WHERE D.idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey 
	AND D.idfQuantity > 0
	AND idfFlagSplit = 0
...
INSERT INTO @tblGLJournalDtl (idfTableLinkKey,idfTableLinkName, idfAmount, idfAmountHome, idfType, idfGLAccountKey, idfGLAccountKey_PAY, idfFlagOverlayGL, idfFlagOverlay_PAY, idfWCOrganizationKey
			...
			,idfGLSegmentDtlKey14
			,idfGLSegmentDtlKey15)
SELECT	 D.idfEXPExpenseSheetDtlHistKey idfEXPExpenseSheetDtlKey
		,'EXPExpenseSheetDtl'
...
WHERE D.idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey AND D.idfFlagSplit = 0
...
WHERE D.idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey AND D.idfFlagSplit = 1
...
SELECT	 ISNULL(S.idfEXPExpenseSheetDtlSplitHistKey,D.idfEXPExpenseSheetDtlHistKey)
		,CASE WHEN MAX(D.idfFlagSplit) = 0 THEN 'EXPExpenseSheetDtl' ELSE 'EXPExpenseSheetDtlSplit' END
...
END	
	...
	FROM dbo.EXPExpenseSheetDtlHist	D WITH (NOLOCK)
		...
		INNER JOIN dbo.EXPExpenseSheetDtlTaxHist         TAX	WITH (NOLOCK) ON D.idfEXPExpenseSheetDtlHistKey = TAX.idfEXPExpenseSheetDtlHistKey AND idfFlagTaxIncluded = 1
	WHERE D.idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey AND D.idfFlagSplit = 0
	GROUP BY D.idfEXPExpenseSheetDtlHistKey, RD.idfGLAccountKey,RD.idfFlagPrimary			,RD.idfGLSegmentDtlKey01
			,RD.idfGLSegmentDtlKey02
			,RD.idfGLSegmentDtlKey03
			,RD.idfGLSegmentDtlKey04
			,RD.idfGLSegmentDtlKey05
	...
	FROM dbo.EXPExpenseSheetDtlHist	D WITH (NOLOCK)
		...
		INNER JOIN dbo.EXPExpenseSheetDtlSplitHist		S		WITH (NOLOCK) ON S.idfEXPExpenseSheetDtlHistKey = D.idfEXPExpenseSheetDtlHistKey
		LEFT OUTER JOIN dbo.vwAPVendorSiteAccount ASA_PURCH	WITH (NOLOCK) ON D.idfAPVendorKey = ASA_PURCH.idfAPVendorKey	AND ASA_PURCH.idfGLPostCodeTypeKey=20100	AND ASA_PURCH.idfIVSiteKey = 0
	WHERE D.idfEXPExpenseSheetHdrHistKey = @xnidfEXPExpenseSheetHdrKey AND D.idfFlagSplit = 1 AND S.idfAmtTaxIncludedHome <> 0
	GROUP BY S.idfEXPExpenseSheetDtlSplitHistKey, S.idfGLAccountKey
...
IF (@nIVLANDCOSTINCLMISC=0 OR @nEXPLANDCOSTINCLMISC=0)
	...
	(
		SELECT ISNULL(S.idfEXPExpenseSheetDtlSplitHistKey, D.idfEXPExpenseSheetDtlHistKey) idfTableLinkKey,
			CASE WHEN D.idfFlagSplit = 1 THEN 'EXPExpenseSheetDtlSplit' ELSE 'EXPExpenseSheetDtl' END idfTableLinkName,
...
IF EXISTS(SELECT TOP 1 1 FROM @tblGLJournalDtl WHERE idfAmount <> 0 ) 
 BEGIN
	...
	--ELSE
	SET @nInterCo = 0
--- AA: NO NEED TO CREATE HEADER AS HEADER IS ALREADY EXISTS
	...
	--	,H.idfEXPExpenseSheetHdrKey			-- idfDocument01
	--	,''										-- idfDocument02
	--	,@nInterCo								-- idfFlagIC
	...
	--	,''										-- idfJournalNumber
	--	,0										-- idfRateHome
	--	,'EXPExpenseSheetHdr'					-- idfTableLinkName
	--	,H.idfEXPExpenseSheetHdrKey			-- idfTableLinkKey
	...
	--	,''--udfTextField04
	--	,''--udfTextField05                                                                                                                                                                                                                                                
	--FROM dbo.EXPExpenseSheetHdr	H  WITH (NOLOCK) 
	...
	--WHERE H.idfEXPExpenseSheetHdrKey = @xnidfEXPExpenseSheetHdrKey 	
...
-- AA  UNTIL HERE
     ...
     SELECT @nidfGLJournalHdrKey = idfGLJournalHdrKey FROM GLJournalHdr where idfTableLinkName='EXPExpenseSheetHdr' and idfDocument01=@xnidfEXPExpenseSheetHdrKey;
	...
	WITH expense AS (
		SELECT 
			...
			,CASE WHEN idfFlagSplit = 0 THEN 'EXPExpenseSheetDtl' ELSE 'EXPExpenseSheetDtlSplit'  END idfTableLinkName
	...
	INSERT INTO #GLJournalDtl (
		...
		,T.idfTableLinkName
		,T.idfTableLinkKey 
		,ISNULL(DSHS.idfGLSegmentDtlKey01,T.idfGLSegmentDtlKey01) idfGLSegmentDtlKey01
		,ISNULL(DSHS.idfGLSegmentDtlKey02,T.idfGLSegmentDtlKey02) idfGLSegmentDtlKey02
		,ISNULL(DSHS.idfGLSegmentDtlKey03,T.idfGLSegmentDtlKey03) idfGLSegmentDtlKey03
		,ISNULL(DSHS.idfGLSegmentDtlKey04,T.idfGLSegmentDtlKey04) idfGLSegmentDtlKey04
		,ISNULL(DSHS.idfGLSegmentDtlKey05,T.idfGLSegmentDtlKey05) idfGLSegmentDtlKey05
		,ISNULL(DSHS.idfGLSegmentDtlKey06,T.idfGLSegmentDtlKey06) idfGLSegmentDtlKey06
		,ISNULL(DSHS.idfGLSegmentDtlKey07,T.idfGLSegmentDtlKey07) idfGLSegmentDtlKey07
		,ISNULL(DSHS.idfGLSegmentDtlKey08,T.idfGLSegmentDtlKey08) idfGLSegmentDtlKey08
		,ISNULL(DSHS.idfGLSegmentDtlKey09,T.idfGLSegmentDtlKey09) idfGLSegmentDtlKey09
		,ISNULL(DSHS.idfGLSegmentDtlKey10,T.idfGLSegmentDtlKey10) idfGLSegmentDtlKey10
		,ISNULL(DSHS.idfGLSegmentDtlKey11,T.idfGLSegmentDtlKey11) idfGLSegmentDtlKey11
		,ISNULL(DSHS.idfGLSegmentDtlKey12,T.idfGLSegmentDtlKey12) idfGLSegmentDtlKey12
		,ISNULL(DSHS.idfGLSegmentDtlKey13,T.idfGLSegmentDtlKey13) idfGLSegmentDtlKey13
		,ISNULL(DSHS.idfGLSegmentDtlKey14,T.idfGLSegmentDtlKey14) idfGLSegmentDtlKey14
		,ISNULL(DSHS.idfGLSegmentDtlKey15,T.idfGLSegmentDtlKey15) idfGLSegmentDtlKey15
		,NULL
		,DH.idfWCDeptKey
	FROM @tblGLJournalDtl T
		...
		LEFT OUTER JOIN dbo.EXPExpenseSheetDtlSegmentHist	DSH     WITH (NOLOCK) ON T.idfTableLinkKey = DSH.idfEXPExpenseSheetDtlHistKey AND T.idfTableLinkName = 'EXPExpenseSheetDtl'
		LEFT OUTER JOIN dbo.EXPExpenseSheetDtlSplitSegmentHist	DSHS     WITH (NOLOCK) ON T.idfTableLinkKey = DSHS.idfEXPExpenseSheetDtlSplitHistKey AND T.idfTableLinkName = 'EXPExpenseSheetDtlSplit'
    ...
    IF EXISTS(SELECT TOP 1 1 FROM dbo.WCSystemSetting WITH (NOLOCK) WHERE idfSettingID='GLAUTOPOSTJRNL' AND idfValue = '1') AND ISNULL(@xnPostGL,0) = 0 BEGIN
	    ...
	    FROM GLJournalHdr
		WHERE idfTableLinkName='EXPExpenseSheetHdr' and idfDocument01=@xnidfEXPExpenseSheetHdrKey
	...
	--FROM dbo.GLJournalHdr H WITH (NOLOCK) 
	--INNER JOIN dbo.GLJournalDtl D WITH (NOLOCK) ON H.idfGLJournalHdrKey = D.idfGLJournalHdrKey
	--INNER JOIN APVoucherDtl DTL WITH (NOLOCK)  ON DTL.idfAPVoucherKey
...
IF(ISNULL(@xonErrNum,0) <> 0) 
	SELECT @xonErrNum
		...
		select * from GLJournalDtl where idfGLJournalHdrKey in (22190) -- Not exists
ROLLBACK TRAN